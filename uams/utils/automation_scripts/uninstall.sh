...
## kill process and restore the config and uninstall uams
uninstall_uams()
{
    names="uwsgi nginx"
    for name in ${names}
    ...
    then	
        sudo cp /opt/uams/utils/automation_scripts/nginx_default.conf /etc/nginx/nginx.conf 
    fi

    if [ -f /etc/nginx/sites-available/nginx_uams ]
    then
        sudo rm /etc/nginx/sites-available/nginx_uams
    fi
...
uninstall_pip()
{
    tmp_log="/tmp/uams_tmp.log"
    packs=`cat $HOME/uams_output/install*.log | grep "pip install success" | uniq | sed 's/\ .*$//'`
    for pack in $packs
    do
        for((num=1;num<=3;num++))
        do
...
uninstall_pkg()
{
    ...
    if [ $system_os = "ubuntu" ]
    then
        packs=`cat $HOME/uams_output/install*.log | grep "pkg install success" | uniq | sed 's/\ .*$//'`
        for pack in $packs
        do
            for((num=1;num<=3;num++))
            do
...
## 2. create log file
    ...
    log="$HOME/uams_output/uninstall_$(date +%Y-%m-%d_%H-%M-%S).log"
    if [ ! -f $log ]
    then 
        touch $log
    fi
...
## 5. uninstall caliper
    ...
    echo "[uninstall_uams]" >> $log
    {
    	uninstall_uams
	echo " uams uninstall success" >> $log
    } | whiptail --title "Uninstall" --gauge "Uninstall uams" 7 55 20
...
## 8. remove output 
    ...
    whiptail --yesno "Please choose remove ~/uams_output" --defaultno 7 58
    choose_status=$?
...
## 9. finish
   whiptail --title "uninstall uams" --msgbox "             uninstall uams finished." 7 58 
   exit 0
}
