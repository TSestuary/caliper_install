...
env_config="/tmp/uams/utils/automation_scripts/environment.config"
depend_json="/tmp/uams/utils/automation_scripts/dependency.json" 
...
##	2. tar source code to /tmp
    ...
    . /tmp/uams/utils/automation_scripts/install_functions
...
##	4. write the new version number to the log file 
    ...
    new_version=`cat /tmp/uams/common.py | grep -owP "VERSION=\K\S+" | sed 's/\"//g'`
    write_log "[uams]" $log
...
## 13. copy uninstall.sh to $HOME/uams_output and copy code to /opt
    ...
    write_log "[install_uams]" $log 
    {
        ...
        then
            sudo cp /tmp/uams/utils/automation_scripts/uninstall.sh $HOME/uams_output/
        else 
