...
## 1. check if caliper is installed
    which caliper
    if [ $? -ne 0 ]; then
	whiptail --title "Error" --msgbox "Could not find caliper." 9 50
#	exit 1
    fi
...
## 8. remove output and source
    ...
    whiptail --yesno "Please choose remove ~/caliper_output and /opt/Caliper" 9 55
    choose_status=$?
...
## 9. finish
    whiptail --title "uninstall caliper" --msgbox "uninstall caliper finished." 9 50  
    exit 0
}
