...
env_config="/tmp/CaliperServer/environment.config"
depend_json="/tmp/CaliperServer/dependency.json" 
...
##	2. tar source code to /tmp
    ...
    . /tmp/CaliperServer/install_functions
...
##	4. write the new version number to the log file 
    ...
    new_version=`cat /tmp/CaliperServer/caliperserver/common.py | grep -owP "VERSION=\K\S+" | sed 's/\"//g'`
...
## 13. copy uninstall.sh to $HOME/caliperserver_output and copy code to /opt
    ...
    write_log "[install_caliperserver]" $log 
    {
        ...
        then
            sudo cp /tmp/CaliperServer/uninstall.sh $HOME/caliperserver_output/
        else 
